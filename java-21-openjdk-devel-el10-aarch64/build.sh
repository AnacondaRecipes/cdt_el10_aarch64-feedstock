...
mkdir -p ${PREFIX}/aarch64-conda_el10-linux-gnu/sysroot
...
pushd ${PREFIX}/aarch64-conda_el10-linux-gnu/sysroot > /dev/null 2>&1
...
for sl in usr/share/systemtap/tapset/arm64/*.stp; do
  link=$(readlink ${sl})
  unlink ${sl}
  ln -s ${PREFIX}/aarch64-conda_el10-linux-gnu/sysroot${link} ${sl}
done
